...
|------|----------|-------------|
| [idasql](https://github.com/0xeb/idasql) | IDA Pro | SQL interface for IDA — query functions, xrefs, strings, types, and decompiler output via SQL. Used in `PoisonPlug_1`. |
| [ghidrasql](https://github.com/0xeb/ghidrasql) | Ghidra | SQL interface for Ghidra — same query-driven workflow, headless HTTP mode, write operations (rename, retype, comment). Used in `sysinternals_handle64`. |
